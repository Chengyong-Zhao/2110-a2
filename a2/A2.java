 ...
 *
 * If a method is called with arguments that do not satisfy the
 * Preconditions,<br>
 * the behavior is undefined (the method can do anything). You do not have to
 * use assert<br>
...
public class A2 {
	/*
	 * Each function you write has a "//TODO comment". Look on the right; click a
	 * blue rectangle to get to the corresponding "//TODO comment". DO NOT DELETE
	 * THESE COMMENTS. Put your code AFTER the comments.
	 *
	 ...
	 * can be eliminated by restructuring/reorganizing code --perhaps writing extra
	 * methods.
	 *
	 * For some functions, you may be writing a loop to append character after
	 * character to an initially empty string. See the JavaHyperText entry for class
	 * StringBuilder and a discussion of why it may be better to use StringBuilder
	 * for this purpose. For this assignment, use either String or StringBuilder, it
	 * doesn't matter which you use.
	 *
	 * We give complete test cases except for the last two methods. You need
	 * practice in thinking about how to test well.
	 */
	...
	/**
	 * Return true iff (i.e. if and only if) the middle characters of s are
	 * different. <br>
	 * Note: If s has an odd number of chars, there is ONE middle char, so return
	 * false.<br>
	 * If s has an even number of chars, there are two middle chars, so return true
	 * only <br>
	...
	public static boolean areMidsDiff(String s) {
		...
		if (s.length() == 0 || s.length() % 2 != 0) { // null or odd
			return false;
		} else { // even
	...
	public static String protectLittles(String s) {
		/*
		 * TODO 2. 1. The spec does NOT say to handle all lower-case letters differently
		 * from the rest, but only those in 'a'..'z'. There are others lower-case
		 * letters!
		 *
		 * 2. In the fourth example, s = "å", 'å' is NOT a character in a..z, so it is
		 * NOT surrounded by anything in the result.
		 *
		 * If this isn't working for you, you may be using Eclipse on a Windows 10
		 * computer, and the wrong Text File coding is being used. Everyone, please
		 * check that you are using the right one by using menu item Preferences ->
		 * General -> Workspace and selecting UTF-8.
		 *
		 ...
		 *
		 * You will lose all points on this question if your code handles all lower-case
		 * letters and not just those in a..z.
		 *
		 * 3. Do NOT use "magic numbers" ---look that term up in JavaHyperText. For
		 * example, the internal representation of 'a' is 97, but do NOT use 97 in the
		 * method body.
		 */
		String REGEX = "[a-z]";
		...
		while (m.find()) {
			m.appendReplacement(sb, "." + m.group() + m.group().toUpperCase());
		}
		m.appendTail(sb); // Implements a terminal append-and-replace step.
		return sb.toString();
	}
	 ...
	 * putCapsLast("ABCDE.FGHIJKLMNO$PQ%RSTUV!WXYZ") =
	...
	public static String putCapsLast(String s) {
		...
		m.appendTail(sb1); // Implements a terminal append-and-replace step. Append tail.
	...
	public static boolean areAnagrams(String s, String t) {
		// TODO 5
		/*
		 * Do not use a loop or recursion! This can be done in 5 lines using methods of
		 * classes String and Arrays
		 * (https://docs.oracle.com/en/java/javase/11/docs/api/java.base/java/util/
		 * Arrays.html). Hint: how can a sequence of characters be uniquely ordered? You
	...
	public static int nCat(String s, String x) {
		...
		if (s == "") {
			return 0;
		} else if ((s.length() > 0 && x.equals("")) || s.length() % x.length() != 0) {
			return -1;
		} else if (x.length() != 0 && x.repeat(s.length() / x.length()).equals(s)) {
	...
	public static int shortest(String s) {
		...
		for (int i = 0; i < ch.length; i++) {
			...
			// System.out.println(nCat(s, compare.substring(0, i)));
			if (nCat(s, compare) > 0) {
